...
class SudokuGenerator {
  static SudokuLevel generateRandomLevel({required int id, required String difficulty, int? seed}) {
    final random = Random(seed);
    
    // 1. まず完成した盤面を作る
    List<List<int>> solution = _generateSolvedGrid(random);
    ...
    _removeCellsSmartly(initial, targetClues, random);
  ...
  static List<List<int>> _generateSolvedGrid(Random random) {
  ...
  static void _removeCellsSmartly(List<List<int>> grid, int targetClues, Random random) {
    List<int> positions = List.generate(81, (i) => i)..shuffle(random);
    int currentClues = 81;
